...
// ============================================================
const Model = {
  // state = het geheugen van de app: alles wat de huidige scan beschrijft.
  // Handig omdat View en Controller hier altijd de actuele gegevens kunnen ophalen.
  state: {
    photoUrl: null,    // de geüploade foto (als data-URL); null = nog geen foto gekozen
    plantType: null,   // gekozen gewas-categorie op stap 2 (bv. 'groente')
    phase: null,       // gekozen groeifase op stap 3 (bv. 'bloei')
    lang: 'nl',        // actieve taal van de interface; start standaard op Nederlands
    reportId: null,    // uniek rapportnummer dat per scan wordt gegenereerd
    potential: null    // berekend groeipotentieel in procenten
  },
  // Een Set i.p.v. array, zodat een doel niet per ongeluk dubbel kan voorkomen.
  selectedGoals: new Set(),

  // localStorage configuratie — sleutel + maximum aantal bewaarde scans.
  HISTORY_KEY: 'auxine-scan-history',  // naam waaronder de geschiedenis wordt opgeslagen
  MAX_HISTORY: 20,                     // niet meer dan 20 scans bewaren, anders loopt opslag vol

  // Vertalingen voor NL en EN
  // ----------------------------------------------------------
  // translations = alle teksten van de interface in NL en EN.
  // Per taal staat hier elke kop, knop, label en lijst die op
  // het scherm verschijnt. Door alles hier centraal te houden,
  // hoeft er nergens anders in de code tekst "hardcoded" te staan
  // en kan de taal in één keer omgezet worden via toggleLang().
  // (Dit blok bevat alleen teksten, geen logica.)
  // ----------------------------------------------------------
  translations: {
    nl: {
  ...
  // --- Getters --- (alleen lézen uit de state, niks veranderen)

  // Haalt het juiste vertaalobject op voor de actieve taal.
  // Zo hoeft de rest van de code niet te weten of het NL of EN is.
  getT() { return this.translations[this.state.lang]; },

  // Geeft de gekozen doelen terug als gewone array (makkelijker om mee te werken dan een Set).
  getSelectedGoals() { return Array.from(this.selectedGoals); },

  // Geeft het aantal gekozen doelen terug; gebruikt voor de teller op stap 4.
  goalCount() { return this.selectedGoals.size; },

  // --- Setters / mutations --- (waarden in de state aanpassen)

  setPhotoUrl(url) { this.state.photoUrl = url; },   // bewaart de geüploade foto
  setPlantType(v) { this.state.plantType = v; },     // bewaart de gewas-keuze van stap 2
  setPhase(v) { this.state.phase = v; },             // bewaart de fase-keuze van stap 3

  // Vinkt een doel aan of uit. Stond het er al in? Dan eruit halen (return false).
  // Stond het er nog niet in? Dan toevoegen (return true). Handig om de UI te laten
  // weten of het vinkje aan of uit moet.
  toggleGoal(v) {
    if (this.selectedGoals.has(v)) { this.selectedGoals.delete(v); return false; }
    this.selectedGoals.add(v); return true;
  },

  clearGoals() { this.selectedGoals.clear(); },   // verwijdert alle gekozen doelen in één keer

  // Zet een vaste lijst doelen klaar (gebruikt bij het terughalen van een oude scan).
  // De || [] vangt het geval op dat er per ongeluk niets wordt meegegeven.
  setGoals(arr) {
    this.selectedGoals.clear();
    (arr || []).forEach(g => this.selectedGoals.add(g));
  },

  // Wisselt tussen Nederlands en Engels en geeft de nieuwe taal terug.
  toggleLang() {
    this.state.lang = this.state.lang === 'nl' ? 'en' : 'nl';
    return this.state.lang;
  },

  // --- Business logica --- (de berekeningen en regels van de app)

  // Maakt een uniek rapportnummer, bv. "A57-2026": letter A + willekeurig getal + jaartal.
  // Zo heeft elke scan een herkenbaar kenmerk dat ook in de e-mail terugkomt.
  generateReportId() {
  ...
  // Berekent het groeipotentieel-percentage op basis van de gekozen doelen.
  calculatePotential() {
    // Elk doel heeft een eigen "waarde" — hoeveel winst het ongeveer oplevert.
    const scores = {
    ...
    // Zet de gekozen doelen om naar hun scores; filter(Boolean) gooit onbekende eruit.
    const nums = goals.map(g => scores[g]).filter(Boolean);
    // Neem de hoogste score als basis; geen doelen gekozen? Dan standaard 20.
    const maxNum = nums.length ? Math.max(...nums) : 20;
    // Kleine bonus voor extra doelen (max +3), zodat meer doelen iets meer potentieel geeft.
    const bonus = Math.min(Math.max(goals.length - 1, 0), 3);
    this.state.potential = maxNum + bonus;
    return this.state.potential;
  },

  // Stelt de persoonlijke adviestekst samen door {plant} en {phase} in de
  // vertaalde sjabloon-zin te vervangen door de echte labels.
  generateDiagnosisText() {
    ...
    // Bij meer dan 1 doel een extra zin toevoegen over het gecombineerde voordeel.
    if (n > 1) txt += t.adv_extra_goals.replace('{n}', n);
    return txt;
  },

  // Kiest de juiste ondertitel onder de scorecard:
  // - precies 1 doel  -> de specifieke uitleg bij dat doel
  // - meerdere doelen -> de gecombineerde tekst met aantal
  // - geen doel       -> de standaardtekst
  ...
  // Controleert of een e-mailadres er geldig uitziet (iets@iets.iets).
  // Voorkomt dat een gebruiker een onzin-adres kan versturen.
  isValidEmail(addr) {
    return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test((addr || '').trim());
  },

  // --- E-mail body opbouwen voor mailto-link ---
  // Bouwt de complete tekst van het e-mailrapport op uit de scan-gegevens.
  buildEmailBody(userMsg) {
    const t = this.getT();
    const plantLabel = t.plant_labels[this.state.plantType] || t.adv_default_plant;
    const phaseLabel = t.phase_labels[this.state.phase] || t.adv_default_phase;
    // Zet de doel-codes om naar leesbare labels, gescheiden door komma's; geen doelen -> "—".
    const goals = this.getSelectedGoals().map(g => t.opts[g] ? t.opts[g].l : g).join(', ') || '—';
    // Haal eventuele HTML-tags (zoals <strong>) uit de adviestekst, want e-mail is platte tekst.
    const diagnosis = this.generateDiagnosisText().replace(/<[^>]+>/g, '');
    // Voeg het optionele bericht van de gebruiker toe, alleen als er echt iets is ingevuld.
    const extra = userMsg && userMsg.trim() ? '\n\n' + userMsg.trim() : '';
    // Elke regel als los stuk; .join('\n') plakt ze met witregels tot één nette e-mailtekst.
    return [
  ...
  // Haalt de opgeslagen scans op uit de browser. try/catch vangt het geval op dat
  // de data corrupt of onleesbaar is — dan geven we gewoon een lege lijst terug
  ...
  // Voegt een nieuwe scan vooraan toe (unshift) zodat de nieuwste bovenaan staat,
  // en knipt de lijst af op MAX_HISTORY zodat opslag niet eindeloos groeit.
  saveToHistory(scan) {
    try {
  ...
  // Wist de hele geschiedenis (gebruikt door de "Geschiedenis wissen"-knop).
  clearHistory() {
    try { localStorage.removeItem(this.HISTORY_KEY); } catch (e) { }
  },

  // Zoekt één specifieke scan op rapport-ID terug in de geschiedenis.
  findScan(id) { return this.loadHistory().find(s => s.id === id); },

  // Sla de huidige scan op vanuit de state — bundelt alle gegevens in één object.
  saveCurrentScan() {
    this.saveToHistory({
      id: this.state.reportId,
      date: new Date().toISOString(),   // tijdstip in standaardformaat, voor sortering/weergave
  ...
  // Laad een opgeslagen scan terug in de state, zodat een oude scan opnieuw
  // getoond kan worden alsof die net is uitgevoerd.
  restoreScan(scan) {
  ...
  // Reset state voor een nieuwe scan (taal blijft staan).
  // Dit lost de bug op waarbij oude gegevens bleven hangen bij "Nieuwe scan starten".
  reset() {
    this.state.photoUrl = null;
    this.state.plantType = null;
    this.state.phase = null;
    this.state.reportId = null;
    this.state.potential = null;
    this.selectedGoals.clear();
  }
};
